...
(function () {
  ...
  // Extrae el código ARC-XXXX de una card (texto o elementos HTML) y devuelve sus dígitos.
  function tokArcCode(elOrText) {
    if (!elOrText) return null;
  ...
  function tokSkuMatch(elOrText, sku) {
    ...
    if (!arc) return false;
    if (arc === d || arc.endsWith(d) || d.endsWith(arc)) return true;
    if (arc.length > d.length && arc.slice(-d.length) === d) return true;
    return false;
  }
  ...
  async function tokProcessCard(cand, it, target, wantType, wantUnit, wantedGrams, wantQty, byName) {
    ...
    if (wantQty > 0) {
      ...
      if (convertedQty > 0 && actualQty < wantQty) {
        ...
        out.usedUnit = usedUnit;
        if (convertedQty > 0) {
          out.message = "agregado: " + qty + " " + wantUnit + " (" + convertedQty + " " + usedUnit + ")";
  ...
  // v2.0.40: vaciar el carrito usando el botón "Borrar carrito" del store.
  // Si no existe ese botón (carrito ya vacío o interfaz distinta), fallback seguro.
  async function tokEmptyCart() {
    try {
      // Abrir el drawer si está cerrado para que el botón sea clickeable.
      let visible = false;
      ...
      if (!visible) {
        ...
        if (btn) { btn.click(); await toksleep(900); }
      }

      // Intentar el botón nativo "Borrar carrito" / "Vaciar carrito" del store.
      const clearSelectors = [
        "[data-id=cart-clear-button]",
        "[data-id=clear-cart-button]",
        "[data-id=empty-cart-button]",
        "button[class*=clear-cart]",
        "button[class*=empty-cart]",
        "button[class*=clearCart]",
        "button[class*=emptyCart]",
      ];
      let clearBtn = null;
      for (const sel of clearSelectors) {
        clearBtn = document.querySelector(sel);
        if (clearBtn) break;
      }
      // Fallback: buscar por texto visible del botón
      if (!clearBtn) {
        const allBtns = Array.from(document.querySelectorAll("button, a[role=button]"));
        clearBtn = allBtns.find((b) => /borrar\s+carrito|vaciar\s+carrito|limpiar\s+carrito|clear\s+cart/i.test(b.innerText || ""));
      }
      if (clearBtn) {
        clearBtn.click();
        await toksleep(1200);
        // Confirmar si hay un modal de confirmación ("Sí" / "Confirmar" / "Aceptar")
        await toksleep(300);
        const confirmBtns = Array.from(document.querySelectorAll("button, a[role=button]"));
        const confirmBtn = confirmBtns.find((b) => /^(s[ií]|yes|confirm|aceptar|ok)$/i.test((b.innerText || "").trim()));
        if (confirmBtn) { confirmBtn.click(); await toksleep(800); }
        return;
      }

      // Fallback final: setear 0 ítem por ítem si el botón no existe.
